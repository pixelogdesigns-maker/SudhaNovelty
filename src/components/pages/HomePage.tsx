...
// --- NEW HELPER: Handles the Instagram Embed Script ---
const InstagramEmbed = ({ rawHtml }: { rawHtml: string }) => {
  useEffect(() => {
    // This function ensures the Instagram script finds and renders the new HTML
    const processEmbeds = () => {
      // @ts-ignore
      if (window.instgrm) {
        // @ts-ignore
    ...
    if (!document.getElementById('react-instagram-embed-script')) {
      ...
      // If script is already loaded, just process the new embed
      processEmbeds();
    }
  }, [rawHtml]);
  ...
  return (
    <div 
      className="instagram-embed-wrapper bg-white rounded-xl overflow-hidden shadow-md mx-auto"
      // Safely render the blockquote string
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-white font-paragraph selection:bg-primary selection:text-white overflow-x-clip">
      ...
      {/* --- Our Videos Section (UPDATED: INSTAGRAM EMBEDS) --- */}
      <section id="videos" className="py-12 bg-gradient-to-b from-white to-light-pink/20 relative overflow-hidden">
        ...
        <style>{`
          ...
          .marquee-container {
            width: fit-content;
            animation: scroll 60s linear infinite; /* Slowed down for readability */
          }
          .marquee-container:hover {
            animation-play-state: paused;
          }
           /* Force width on embeds to ensure they don't collapse */
          .instagram-embed-wrapper {
             width: 330px !important;
             flex-shrink: 0;
          }
          /* Override Instagram's min-width if necessary */
          .instagram-media {
            ...
            >
                ...
                {[...Array(2)].map((_, loopIndex) => (
                  <div key={loopIndex} className="flex gap-6 shrink-0">
                    ...
                    {/* RENDERING 6 COPIES OF THE SUDHA VIDEO 
                      To change these, duplicate the 'sudhaVideoHTML' variable with new code
                      and map through an array of those strings.
                    */}
                    {[...Array(6)].map((_, i) => (
                       <div key={`${loopIndex}-${i}`} className="w-[330px]">
                          <InstagramEmbed rawHtml={sudhaVideoHTML} />
                       </div>
                    ))}
