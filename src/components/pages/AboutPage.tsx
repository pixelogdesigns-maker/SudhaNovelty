...
const HeroCarousel = () => {
  ...
  useEffect(() => {
    ...
    // When reaching the last cloned slide
    ...
    // When going backwards past the first cloned slide
    else if (index === 0) {
      setTimeout(() => {
        track.style.transition = 'none';
...
const BestSellers = ({ toys }: { toys: Toys[] }) => {
  ...
  // Use explicit casting to avoid 'any' if possible, or keep it safe for now
        ...
        <div className="flex justify-between items-end mb-12">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`p-4 rounded-full border border-gray-200 bg-white transition-all ${!canScrollPrev ? 'opacity-50 cursor-not-allowed' : 'hover:bg-primary hover:text-white'}`}
            >
              <ChevronLeft size={24} />
            </button>
            <button
              ...
              className={`p-4 rounded-full border border-gray-200 bg-white transition-all ${!canScrollNext ? 'opacity-50 cursor-not-allowed' : 'hover:bg-primary hover:text-white'}`}
            >
